...
void parseArgument(vector<string> token, int argc,int start,char** argv){
    for(int i=0;i<argc;i++,start++){
        ...
        sprintf(argv[i],"%s",token[start].c_str());
    }
    argv[argc] = NULL;
}
...
void parseCommand(vector<string> token){
    ...
    while(tail<token.size()){
        ...
        while(tail<token.size()&&!isOrdinPipe(token[tail])&& !isExclamation(token[tail])&&!isNumberPipe(token[tail])&&!isWriteFile(token[tail])){
            tail++;
        }
        if(isFirst){
